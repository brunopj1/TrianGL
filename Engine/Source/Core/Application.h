...
namespace TGL
{
    ...
    void Application::Run(const ApplicationConfig& config, const ServiceCollection& mockServiceCollection, Args&&... args) // NOLINT(cppcoreguidelines-missing-std-forward)
    {
        ...
        if (mockServiceCollection.Clock == nullptr) serviceCollection.Clock = new Clock();
        if (mockServiceCollection.Window == nullptr) serviceCollection.Window = new Window();
        if (mockServiceCollection.InputSystem == nullptr) serviceCollection.InputSystem = new InputSystem();
        if (mockServiceCollection.EntityManager == nullptr) serviceCollection.EntityManager = new EntityManager();
        if (mockServiceCollection.AssetManager == nullptr) serviceCollection.AssetManager = new AssetManager();
        ...
        if (mockServiceCollection.Clock == nullptr) delete serviceCollection.Clock; // NOLINT(readability-delete-null-pointer)
        if (mockServiceCollection.Window == nullptr) delete serviceCollection.Window; // NOLINT(readability-delete-null-pointer)
        if (mockServiceCollection.InputSystem == nullptr) delete serviceCollection.InputSystem; // NOLINT(readability-delete-null-pointer)
        if (mockServiceCollection.EntityManager == nullptr) delete serviceCollection.EntityManager; // NOLINT(readability-delete-null-pointer)
        if (mockServiceCollection.AssetManager == nullptr) delete serviceCollection.AssetManager; // NOLINT(readability-delete-null-pointer)
    }
#endif
}
