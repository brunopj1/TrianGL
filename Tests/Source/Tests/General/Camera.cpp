...
BEGIN_GAME_TEST_MOCKED(Camera, Render, MockServiceBuilder)
{
	void OnUpdate(f32 deltaTime) override
	{
		...
		if (frame == 3)
		{
			EXPECT_NE(Camera::GetMainCamera(), nullptr);
...
BEGIN_GAME_TEST(Camera, Size)
{
	void OnUpdate(f32 deltaTime) override
	{
		...
		EXPECT_NEAR(window.GetAspectRatio(), 2.0f, 0.0001f);
		EXPECT_EQ(window.GetAspectRatio(), camera->GetAspectRatio());

		camera->SetHorizontalSize(10.0f);
		const glm::uvec2 size1 = camera->GetSize();
		EXPECT_NEAR(camera->GetHorizontalSize(), size1.x, 0.0001f);
		EXPECT_NEAR(camera->GetVerticalSize(), size1.y, 0.0001f);
		EXPECT_NEAR(size1.x, 10.0f, 0.0001f);
		EXPECT_NEAR(size1.y, 5.0f, 0.0001f);

		camera->SetVerticalSize(10.0f);
		const glm::uvec2 size2 = camera->GetSize();
		EXPECT_NEAR(camera->GetHorizontalSize(), size2.x, 0.0001f);
		EXPECT_NEAR(camera->GetVerticalSize(), size2.y, 0.0001f);
		EXPECT_NEAR(size2.x, 20.0f, 0.0001f);
		EXPECT_NEAR(size2.y, 10.0f, 0.0001f);
...
BEGIN_GAME_TEST(Camera, DepthRange)
{
	void OnUpdate(f32 deltaTime) override
	{
		...
		EXPECT_THROW(camera->SetDepthRange(1000, 1000), std::invalid_argument);
		EXPECT_THROW(camera->SetDepthRange(1000, -1000), std::invalid_argument);
...
BEGIN_GAME_TEST_MOCKED(Camera, BackgroundColor, MockServiceBuilder)
{
	...
	void OnUpdate(f32 deltaTime) override
	{
		...
		if (frame == 1)
		{
			...
			EXPECT_NEAR(color.x, 0.1f, 0.0001f);
			EXPECT_NEAR(color.y, 0.2f, 0.0001f);
			EXPECT_NEAR(color.z, 0.3f, 0.0001f);
		}

		if (frame == 2)
		{
			EXPECT_NEAR(MockRenderBackend::s_ClearColor.x, 0.1f, 0.0001f);
			EXPECT_NEAR(MockRenderBackend::s_ClearColor.y, 0.2f, 0.0001f);
			EXPECT_NEAR(MockRenderBackend::s_ClearColor.z, 0.3f, 0.0001f);
...
BEGIN_GAME_TEST(Camera, CoordinateConversion)
{
	void OnUpdate(f32 deltaTime) override
	{
		...
		EXPECT_NEAR(screenCoords1.x, 250.0f, 0.0001f);
		EXPECT_NEAR(screenCoords1.y, 250.0f, 0.0001f);

		const glm::vec2 screenCoords2 = camera->WorldToScreenPosition({-2.5f, 2.5f});
		EXPECT_NEAR(screenCoords2.x, 125, 0.0001f);
		EXPECT_NEAR(screenCoords2.y, 125, 0.0001f);

		const glm::vec2 worldCoords1 = camera->ScreenToWorldPosition({250.0f, 250.0f});
		EXPECT_NEAR(worldCoords1.x, 0.0f, 0.0001f);
		EXPECT_NEAR(worldCoords1.y, 0.0f, 0.0001f);

		const glm::vec2 worldCoords2 = camera->ScreenToWorldPosition({-1000.0f, 750.0f});
		EXPECT_NEAR(worldCoords2.x, -25.0f, 0.0001f);
		EXPECT_NEAR(worldCoords2.y, -10.0f, 0.0001f);
		...
		EXPECT_NEAR(worldCoords3.x, worldCoords4.x, 0.01f);
		EXPECT_NEAR(worldCoords3.y, worldCoords4.y, 0.01f);
